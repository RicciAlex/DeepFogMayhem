...
CMovingCube* CMovingCube::Create(const D3DXVECTOR3 pos, const D3DXVECTOR3 move, const float fRange)
{
	...
	pCube->SetScale(1.5f);			//スケールの設定

	//ヒットボックスの生成
	pCube->m_pHitbox = CBoxHitbox::Create(pos, D3DXVECTOR3(0.0f, -m_metalBoxSize.y * 1.5f, 0.0f), 
		D3DXVECTOR3(m_metalBoxSize.x * 0.5f * 1.5f, m_metalBoxSize.y * 1.25f, m_metalBoxSize.z * 0.5f * 1.5f), CHitbox::TYPE_NEUTRAL, pCube);
